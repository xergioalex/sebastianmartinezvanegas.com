...
{:else}
  <form on:submit={handleSubmit} class="bg-cream shadow-lg p-4 space-y-3">
    ...
    <button
      type="submit"
      class="w-full bg-black text-white font-semibold py-2 rounded hover:bg-gray-900 transition flex items-center justify-center {isLoading ? 'opacity-60 cursor-not-allowed' : ''}"
      disabled={isLoading}
    >
      {#if isLoading}
